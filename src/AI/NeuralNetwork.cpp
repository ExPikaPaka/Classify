...
NeuralNetwork::NeuralNetwork(int layersCount, int neuronsInLayerCount[]) {
	init(layersCount, neuronsInLayerCount);
}


void NeuralNetwork::init(int layersCount, int neuronsInLayerCount[]) {
	// Pre-initialization
	outputDebugInfo = 0;
...
void NeuralNetwork::forwardPropagation(std::vector<double> inputVector) {
...
void NeuralNetwork::fillInputVector(std::vector<double> inputVector) {
	if (inputVector.size() != layer[0].neuronsCount) return;
...
void NeuralNetwork::fillExpectedVector(std::vector<double> expectedVector) {
	if (expectedVector.size() != layer[layersCount - 1].neuronsCount) return;
